  ...
  {if $myprojects[project].tasknum}
    <div class="headline">
      <a href="javascript:void(0);" id="block-{$myprojects[project].ID}_toggle" class="win_block"
         onclick="toggleBlock('block-{$myprojects[project].ID}');"></a>

      <div class="wintools">
        {if $userpermissions.tasks.add}
          <a class="add" href="javascript:blindtoggle('form_{$myprojects[project].ID}');"
             id="add_{$myprojects[project].ID}"
             onclick="toggleClass(this,'add-active','add');toggleClass('add_butn_{$myprojects[project].ID}','butn_link_active','butn_link');toggleClass('sm_{$myprojects[project].ID}','smooth','nosmooth');"><span>{#addtask#}</span></a>
        {/if}
      </div>

      <h2>
        <a href="managetask.php?action=showproject&amp;id={$myprojects[project].ID}"
           title="{$myprojects[project].name} / {#mytasks#}"><img src="./templates/standard/images/symbols/projects.png"
                                                                  alt=""/>{$myprojects[project].name|truncate:80:"...":true}
        </a>
      </h2>
    </div>
    ...
    <div class="nosmooth" id="sm_{$myprojects[project].ID}">
      <table id="acc_{$myprojects[project].ID}" cellpadding="0" cellspacing="0" border="0">
        ...
        <tr>
          <th class="a"></th>
          <th class="b"><a href="managetask.php?action=showproject&amp;id={$myprojects[project].ID}">{#tasks#}</a>
          </th>
          <th class="c">{#assignto#}</th>
        ...
        {if $myprojects[project].tasknum > 0}
          ...
          {section name=task loop=$myprojects[project].tasks}
            ...
            <tr {if $myprojects[project].tasks[task].daysleft < 0} class="marker-late"{elseif $myprojects[project].tasks[task].daysleft == 0} class="marker-today"{/if}>
              ...
              <td>{$myprojects[project].tasks[task].user|truncate:23:"...":true}</td>
              <td style="text-align:right">{$myprojects[project].tasks[task].daysleft}&nbsp;&nbsp;</td>
              <td class="tools">
                {if $userpermissions.tasks.edit}
                  <a class="tool_edit"
                     href="managetask.php?action=editform&amp;tid={$myprojects[project].tasks[task].ID}&amp;id={$myprojects[project].ID}"
                     title="{#edit#}"></a>
                {/if}
                {if $userpermissions.tasks.del}
                  <a class="tool_del"
                     href="javascript:confirmfunction('{#confirmdel#}','deleteElement(\'task_{$myprojects[project].tasks[task].ID}\',\'managetask.php?action=del&amp;tid={$myprojects[project].tasks[task].ID}&amp;id={$myprojects[project].ID}\')');"
                     title="{#delete#}"></a>
                {/if}
              </td>
            </tr>
      ...
      <div id="doneblock_{$myprojects[project].ID}" class="doneblock" style="display: none;">
        <table class="second-thead" cellpadding="0" cellspacing="0" border="0"
          ...
          <tr>
            <td class="a"></td>
            <td class="b"><span id="toggle-done-{$myprojects[project].ID}" class="acc-toggle">{#donetasks#}</span>
            </td>
            <td class="c"></td>
            <td class="d"></td>
            <td class="tools"></td>
          ...
          <table cellpadding="0" cellspacing="0" border="0" id="done_{$myprojects[project].ID}">
            {section name=oldtask loop=$myprojects[project].oldtasks}
              ...
              <tr>
                ...
                <td class="c"><a
                    href="managetasklist.php?action=showtasklist&amp;&amp;tlid={$myprojects[project].oldtasks[oldtask].liste}&amp;id={$myprojects[project].ID}">{$myprojects[project].oldtasks[oldtask].list|truncate:23:"...":true}</a>
                </td>
                <td class="d" style="text-align:right">{$myprojects[project].oldtasks[oldtask].daysleft}
                  &nbsp;&nbsp;</td>
                <td class="tools">
                  {if $userpermissions.tasks.edit}
                    <a class="tool_edit"
                       href="managetask.php?action=editform&amp;tid={$myprojects[project].oldtasks[oldtask].ID}&amp;id={$myprojects[project].ID}"
                       title="{#edit#}"></a>
                  {/if}
                  {if $userpermissions.tasks.del}
                    <a class="tool_del"
                       href="javascript:confirmfunction('{#confirmdel#}','deleteElement(\'task_{$myprojects[project].oldtasks[oldtask].ID}\',\'managetask.php?action=del&amp;tid={$myprojects[project].oldtasks[oldtask].ID}&amp;id={$myprojects[project].ID}\')');"
                       title="{#delete#}"></a>
                  {/if}
                </td>
              </tr>
    ...
    <div class="tablemenue">
      <div class="tablemenue-in">
        {if $userpermissions.tasks.add}
          <a class="butn_link" href="javascript:blindtoggle('form_{$myprojects[project].ID}');"
             id="add_butn_{$myprojects[project].ID}"
             onclick="toggleClass('add_{$myprojects[project].ID}','add-active','add');toggleClass(this,'butn_link_active','butn_link');toggleClass('sm_{$myprojects[project].ID}','smooth','nosmooth');">{#addtask#}</a>
        {/if}
        <a class="butn_link" href="javascript:blindtoggle('doneblock_{$myprojects[project].ID}');"
           id="donebutn_{$myprojects[project].ID}"
           onclick="toggleClass(this,'butn_link_active','butn_link');toggleClass('toggle-done-{$myprojects[project].ID}','acc-toggle','acc-toggle-active');">{#donetasks#}</a>
      </div>
    </div>
    {*Add Task End*}
