    ...
    if ($project->assign($user, $id))
    {
        if ($settings["mailnotify"])
        {
            ...
            if (!empty($user["email"]))
            {
                ...
                $themail->send_mail($user["email"], $langfile["projectassignedsubject"] , $langfile["projectassignedtext"] . " <a href = \"" . "http://janus.jenkins-asia.com/" . "manageproject.php?action=showproject&id=$id\">" . $url . "manageproject.php?action=showproject&id=$id</a>");
            }
        }
        if ($redir)
        {
...
} elseif ($action == "showproject")
{
    ...
    $template->assign("done", $tproject['progress']);
    ...
    $template->assign("deliverables", $deliverables);
    $myuser = new user();
