...
function() {
    ...
    if (t.getBoundingClientRect) {
        ...
        return e = i.top + r - l, n = i.left + o - d, {
            y: Math.round(e),
            x: Math.round(n),
            width: t.offsetWidth,
    ...
    for (var t = this.getGridColumns(), e = [], n = 0, i = this.locale.labels, a = this.config.scale_height - 2, s = 0; s < t.length; s++) {
        ...
        h = h || "";
        var c = "<div class='" + d + "' style='" + _ + "' column_id='" + o.name + "'>" + h + l + "</div>";
        e.push(c)
    }
    ...
    for (var e = this.getGridColumns(), n = [], i = 0; i < e.length; i++) {
        ...
        if (o.tree) {
            ...
            h ? (d += this.templates.grid_open(t), d += this.templates.grid_folder(t)) : (d += this.templates.grid_blank(t), d += this.templates.grid_file(t))
        }
        var c = "width:" + (o.width - (r ? 1 : 0)) + "px;";
        dhtmlx.defined(o.align) && (c += "text-align:" + o.align + ";"), a = "<div class='" + l + "' style='" + c + "'>" + d + s + "</div>",
        n.push(a)
    }
    var l = t.$index % 2 === 0 ? "" : " odd";
    if (l += t.$transparent ? " gantt_transparent" : "", this.templates.grid_row_class) {
    ...
    if (t != n) {
        ...
        if (this.callEvent("onBeforeTaskMove", [t, n, e]) !== !1) {
            ...
            var l = r[e];
            l ? r = r.slice(0, e).concat([t]).concat(r.slice(e)) : r.push(t), this.setParent(a, n), this._branches[n] = r;
            for (var d = this._getTaskTree(t), _ = 0; _ < d.length; _++) {
                var h = this._pull[d[_]];
    ...
    dhtmlx.defined(this.config.dnd_sensitivity) && (t.config.sensitivity = this.config.dnd_sensitivity), t.attachEvent("onBeforeDragStart", dhtmlx.bind(function(e, n) {
        ...
        if (this.isTaskExists(o) || (o = t.lastTaskOfLevel(l.$level), o == t.config.id && (o = null)), this.isTaskExists(o)) {
            var d = gantt._get_position(r),
                _ = this.getTask(o);
            if (d.y + r.offsetHeight / 2 < s) {
                var h = this.getGlobalTaskIndex(_.id),
                    c = this._pull[this._order[h + 1]];
                if (c) {
                    if (c.id == l.id) return;
                    _ = c
                } else if (c = this._pull[this._order[h]], c.$level == l.$level && c.id != l.id) return void this.moveTask(l.id, -1, this.getParent(c.id))
            }
            for (var h = this.getGlobalTaskIndex(_.id), g = this._pull[this._order[h - 1]], u = 1;
                (!g || g.id == _.id) && h - u >= 0;) g = this._pull[this._order[h - u]], u++;
            if (l.id == _.id) return;
            _.$level == l.$level && l.id != _.id ? this.moveTask(l.id, 0, 0, _.id) : _.$level != l.$level - 1 || gantt.getChildren(_.id).length ? g && g.$level == l.$level && l.id != g.id && this.moveTask(l.id, -1, this.getParent(g.id)) : this.moveTask(l.id, 0, _.id)
        }
        return !0
    ...
    adjustSize: function(t, e, n, i) {
        ...
        for (var a = i - n + 1, s = this.getSum(e, n, i), r = 0, o = n; i >= o; o++) {
            var l = Math.floor(t * (s ? e[o] / s : 1 / a));
            s -= e[o], t -= l, a--, e[o] += l, r += l
        }
        e[e.length - 1] += t
    },
    ...
    prepareConfigs: function(t, e, n, i) {
        for (var a = this.splitSize(i, t.length), s = n, r = [], o = t.length - 1; o >= 0; o--) {
            ...
            l && this.processIgnores(d), this.initColSizes(d, e, s, a[o]), this.limitVisibleRange(d), l && (s = d.full_width), r.unshift(d)
        }
    ...
    iterateScales: function(t, e, n, i, a) {
        for (var s = e.trace_x, r = t.trace_x, o = n || 0, l = i || r.length - 1, d = 0, _ = 1; _ < s.length; _++)
            for (var h = o; l >= h; h++) + r[h] != +s[_] || (a && a.apply(this, [d, _, o, h]), o = h, d = _)
    },
    ...
    limitVisibleRange: function(t) {
        ...
        if (a) {
            for (var d = this.getSum(t.width), _ = 0, h = 0; h < t.width.length; h++) {
                var c = Math.floor(a * (t.width[h] / d));
                t.width[h] += c,
                _ += c
            }
    ...
    on_mouse_down: function(t, e) {
        if (2 != t.button) {
            ...
            if (gantt.isTaskExists(n) && (i = gantt.getTask(n)), !gantt._is_readonly(i) && !this.drag.mode) {
                ...
                if (gantt.checkEvent("onMouseDown") && gantt.callEvent("onMouseDown", [a.split(" ")[0]]) && e.parentNode) return this.on_mouse_down(t, e.parentNode);
            }
        }
    },
    ...
    for (var o = 0; o < i.length; o++) {
        ...
        var l = n.render_line(i[o], i[o + 1]);
        t.color && (l.firstChild.style.backgroundColor = t.color), a.appendChild(l)
    }
    ...
    return n.style.cssText = ["top:" + a + "px", "left:" + s + "px"].join(";"), n.className = l, n
}, gantt._drawer = {
    current_pos: null,
    dirs: {
        left: "left",
    ...
    get_points: function(t) {
        ...
        if (t.type == gantt.config.links.start_to_start) this.point_to(s.left, r), o ? (this.point_to(s.down, i), this.point_to(s.right, a)) : (this.point_to(s.right, a), this.point_to(s.down, i)), this.point_to(s.right, r);
            ...
            if (o = e.e_x > e.x + 2 * r, this.point_to(s.right, r), o) a -= r, this.point_to(s.down, i), this.point_to(s.right, a);
                ...
                a -= 2 * r;
                var l = i > 0 ? 1 : -1;
                this.point_to(s.down, l * (n.row_height / 2)), this.point_to(s.right, a), this.point_to(s.down, l * (Math.abs(i) - n.row_height / 2)), this.point_to(s.right, r)
            } else
        if (t.type == gantt.config.links.finish_to_finish) this.point_to(s.right, r), o ? (this.point_to(s.right, a), this.point_to(s.down, i)) : (this.point_to(s.down, i), this.point_to(s.right, a)), this.point_to(s.left, r);
            ...
            if (o = e.e_x > e.x - 2 * r, this.point_to(s.left, r), o) {
                ...
                this.point_to(s.right, a), this.point_to(s.down, l * (Math.abs(i) - n.row_height / 2)), this.point_to(s.left, r)
    ...
    _.attachEvent("onBeforeDragStart", dhtmlx.bind(function(e, n) {
        ...
        var r = _.config,
            o = _.getPosition(a);
        n(r.marker, o);
        var l = gantt._is_link_drop_area(a),
            d = gantt._link_target_task,
            h = gantt._link_landing,
            g = gantt._link_target_task_start,
            f = gantt.locate(a),
            p = !0;
        if (l && (p = !gantt._locate_css(a, c), l = !! f), gantt._link_target_task = f, gantt._link_landing = l, gantt._link_target_task_start = p, l) {
        ...
        a(), t.from && gantt.refreshTask(t.from, !1), t.to && gantt.refreshTask(t.to, !1), d()
    ...
    for (var s = 0; s < t.count; s++) {
        ...
        var r = i.call(this, n),
            ...
            _ = "";
        if (o) {
            l = "width:" + o + "px;", _ = "gantt_scale_cell" + (s == t.count - 1 ? " gantt_last_cell" : ""),
            d = a.call(this, n), d && (_ += " " + d);
            var h = "<div class='" + _ + "' style='" + l + "'>" + r + "</div>";
            e.push(h)
        }
    }
    return e.join("")
    ...
    if (this._is_chart_visible()) {
        ...
        for (var r = this._get_resize_options(), o = r.x ? Math.max(this.config.autosize_min_width, 0) : this.$task.offsetWidth, l = a.prepareConfigs(s, this.config.min_column_width, o, i), d = this._tasks = l[l.length - 1], _ = [], h = this.templates.scale_row_class, c = 0; c < l.length; c++) {
            var g = "gantt_scale_line",
                u = h(l[c]);
            u && (g += " " + u), _.push('<div class="' + g + '" style="height:' + l[c].height + "px;line-height:" + l[c].height + 'px">' + this._prepare_scale_html(l[c]) + "</div>")
        }
        t = _.join(""), e = d.full_width + this.$scroll_ver.offsetWidth + "px", n = d.full_width + "px", i += "px"
    }
    ...
    if (gantt.config.show_task_cells)
        for (var a = 0; n > a; a++) {
            ...
            if (s > 0) {
                ...
                o.style.width = s + "px", r = "gantt_task_cell" + (a == n - 1 ? " gantt_last_cell" : ""), d = this.templates.task_cell_class(t, e.trace_x[a]), d && (r += " " + d), o.className = r, i.appendChild(o)
            }
        }
    var l = t.$index % 2 !== 0,
        d = gantt.templates.task_row_class(t.start_date, t.end_date, t),
        _ = "gantt_task_row" + (l ? " odd" : "") + (d ? " " + d : "");
    return this.getState().selected_task == t.id && (_ += " gantt_selected"), i.className = _, i.style.height = gantt.config.row_height + "px",
    ...
    if (r.link_source_id == n && i.push("gantt_link_source"), r.link_target_id == n && i.push("gantt_link_target"), this.config.highlight_critical_path && this.isCriticalTask && this.isCriticalTask(s) && i.push("gantt_critical_task"), r.link_landing_area && r.link_target_id && r.link_source_id && r.link_target_id != r.link_source_id) {
        var o = r.link_source_id,
            ...
            h = "";
        h = _ ? d ? "link_start_allow" : "link_finish_allow" : d ? "link_start_deny" : "link_finish_deny", i.push(h)
    }
    return i.join(" ")
    ...
    (t.color || t.progressColor || t.textColor) && (d += " gantt_task_inline_color"),
    s.className = d;
    ...
    for (var n, i, a, s = 0, r = t.length - 1; r >= s;)
        if (n = Math.floor((s + r) / 2), i = +t[n], a = +t[n - 1], e > i) s = n + 1;
        else {
            if (!(i > e)) return n;
...
gantt.removeLinkLayer = function(t) {
    ...
    get_closest_worktime: function(t) {
        ...
        for ("past" == t.dir && (d = -1); !this.is_working_unit(n, e);)
            if (l && (n = s ? i : a, d = -1 * d), n = gantt.date.add(n, d, e), n = gantt.date[e + "_start"](n), l && (s ? i = n : a = n), s = !s, o++, o > r) return dhtmlx.assert(!1, "Invalid working time check"), !1;
    ...
    return dhtmlx.defined(e) || (e = this.getParent(t) || 0), this.isTaskExists(e) || (e = 0), this.setParent(t, e), t = this._init_task(t), this.callEvent("onBeforeTaskAdd", [t.id, t]) === !1 ? !1 : (this._pull[t.id] = t, this._add_branch(t), this.refreshData(), this._adjust_scales(), this.callEvent("onAfterTaskAdd", [t.id, t]), t.id)
...
gantt.createTask = function(t, e) {
    ...
    var s = "string" == typeof t ? function(n, i) {
        ...
        r = this.getChildren(n);
    if (r) {
        for (var o = [], l = r.length - 1; l >= 0; l--) o[l] = this._pull[r[l]];
        o.sort(s);
        for (var l = 0; l < o.length; l++) r[l] = o[l].id, this.sort(t, e, r[l], !0)
    }
    a && this.render()
    ...
    t.afterUpdate = function() {
        ...
        var i = t._ganttMode,
            a = n.filePath || (n.xmlDoc ? n.xmlDoc.responseURL : "");
...
gantt.showLightbox = function(t) {
    ...
    for (var a = this._get_typed_lightbox_config(this.getLightboxType()), s = 0; s < a.length; s++) {
        ...
        if (this.form_blocks[r.type]) {
            var o = document.getElementById(r.id).nextSibling,
                l = this.form_blocks[r.type],
                d = gantt._resolve_default_mapping(a[s]),
                _ = dhtmlx.defined(n[d]) ? n[d] : r.default_value;
            l.set_value.call(gantt, o, _, n, r), r.focus && l.focus.call(gantt, o)
        }
    }
    ...
    getTimePicker: function(t, e) {
        ...
        for (var o = "", l = 0; l < n.length; l++) {
            ...
            switch (d) {
                case "%Y":
                    t._time_format_order[2] = l, t._time_format_order.size++;
                    var h, c, g, u;
                    ...
                    for (var f = g; u > f; f++) _ += "<option value='" + f + "'>" + f + "</option>";
                    break;
                case "%m":
                    t._time_format_order[1] = l, t._time_format_order.size++;
                    for (var f = 0; 12 > f; f++) _ += "<option value='" + f + "'>" + this.locale.date.month_full[f] + "</option>";
                    break;
                case "%d":
                    t._time_format_order[0] = l, t._time_format_order.size++;
                    for (var f = 1; 32 > f; f++) _ += "<option value='" + f + "'>" + f + "</option>";
                    break;
                case "%H:%i":
                    t._time_format_order[3] = l, t._time_format_order.size++;
                    var f = r,
                        p = a.getDate();
                    for (t._time_values = []; s > f;) {
                        var v = this.templates.time_picker(a);
                        _ += "<option value='" + f + "'>" + v + "</option>", t._time_values.push(f), a.setTime(a.valueOf() + 60 * this._get_timepicker_step() * 1e3);
    ...
    time: {
        ...
        set_value: function(t, e, n, i) {
            ...
            var d = gantt._resolve_default_mapping(i);
            "string" == typeof d && (d = {
                start_date: d
            });
            var _ = n[d.start_date] || new Date,
                h = n[d.end_date] || gantt.calculateEndDate(_, 1);
        ...
        get_value: function(t, e, n) {
            ...
            var l = new Date(i[a[2]].value, i[a[1]].value, i[a[0]].value, s, r);
            if (s = r = 0, dhtmlx.defined(a[3])) {
                var o = parseInt(i[a.size + a[3]].value, 10);
                s = Math.floor(o / 60), r = o % 60
            }
            var d = new Date(i[a[2] + a.size].value, i[a[1] + a.size].value, i[a[0] + a.size].value, s, r);
    ...
    duration: {
        ...
        set_value: function(t, e, n, i) {
            ...
            function s(t) {
                var e = d.value;
                e = parseInt(e, 10), window.isNaN(e) && (e = 0), e += t, 1 > e && (e = 1), d.value = e, a()
            }
            var r = i,
            ...
            _[0].onclick = dhtmlx.bind(function() {
                ...
                s(1 * this.config.duration_step)
            }, this), o[0].onchange = a, o[1].onchange = a, o[2].onchange = a, o[3] && (o[3].onchange = a), d.onkeydown = dhtmlx.bind(function(t) {
                t = t || window.event;
            ...
            gantt.form_blocks._fill_lightbox_select(o, 0, u, c, r), d.value = p, a()
        },
...
function() {
    ...
    this._gantt_touch_event_ready || (this._gantt_touch_event_ready = 1, dhtmlxEvent(gantt.$container, t[0], function(t) {
        if (!n(t) && o) {
            _ && clearTimeout(_);
            var a = e(t);
            if (gantt._tasks_dnd.drag.id || gantt._tasks_dnd.drag.start_drag) return gantt._tasks_dnd.on_mouse_move(a),
            t.preventDefault && t.preventDefault(), t.cancelBubble = !0, !1;
            if (a && d) {
                var h = d.pageX - a.pageX,
                    c = d.pageY - a.pageY;
                !l && (Math.abs(h) > 5 || Math.abs(c) > 5) && (gantt._touch_scroll_active = l = !0, r = 0, s = gantt.getScrollState()), l && gantt.scrollTo(s.x + h, s.y + c)
            }
            return i(t)
        }
        ...
        if (!n(t)) {
            ...
            if (o = !0, d = e(t), d && r) {
                var s = new Date;
                500 > s - r ? (gantt._on_dblclick(d), i(t)) : r = s
            } else r = new Date;
            _ = setTimeout(function() {
                var t = gantt.locate(d);
                t && -1 == d.target.className.indexOf("gantt_link_point") && (gantt._tasks_dnd.on_mouse_down(d), gantt._tasks_dnd._start_dnd(d), gantt._touch_drag = !0, a(t), gantt.refreshTask(t), gantt._touch_feedback()), _ = null
        ...
        if (!n(t)) {
            ...
            var i = e(t);
            gantt._tasks_dnd.on_mouse_up(i), h && (gantt.refreshTask(gantt.locate(h)), h.parentNode.removeChild(h), gantt._touch_feedback()), gantt._touch_scroll_active = o = l = !1, h = null
        }
    })
};
