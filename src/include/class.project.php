...
class project {
    ...
    function getProjects($status = 1, $lim = 10)
    {
        ...
        $projekte = array();
        $sel = mysql_query("SELECT ID FROM projekte WHERE `valid`=$status ORDER BY end_date ASC LIMIT $lim");
    ...
    function getProjectMembers($project, $lim = 10, $paginate = true)
    {
        ...
        $sql1 = mysql_query("SELECT p.ID,p.user,p.projekt as project,u.name,u.title,'sql changed' as location,u.email,u.tel1,u.tel2 FROM user u,projekte_assigned p where p.user = u.id and  p.projekt = $project LIMIT $start,$lim");
