...
class project {
    ...
    function getProjectMembers($project, $lim = 10, $paginate = true)
    {
        ...
        $sql1 = mysql_query("SELECT p.ID,p.user,u.role_type,p.projekt as project,u.name,u.title,'sql changed' as location,u.email,u.tel1,u.tel2 FROM user u,projekte_assigned p where p.user = u.id and  p.projekt = $project LIMIT $start,$lim");
