...
//              $project["done"] = $this->getProgress($project["ID"]);
  ...
  function getProjectBeans(){
    ...
    function getProjectMembers($project, $lim = 1000, $paginate = true)
    {
        ...
        $sql1 = mysql_query("SELECT p.ID,p.user, u.ID as user_id,u.role_type,p.projekt as project,u.name,u.title,'sql changed' as location,u.email,u.tel1,u.tel2 FROM user u,projekte_assigned p where p.user = u.id and u.is_valid = 1 and p.projekt = $project LIMIT $start,$lim");
