...
class Imap {
    ...
    public function getFolderIds() {
    	...
    	$folderIds=array();
    	foreach($folders as $folderStr){
    		array_push($folderIds,str_replace($this->mailbox, "", $folderStr));
    	}
    ...
    protected function formatMessage($id, $withbody=true){
        ...
        $email = array(
            ...
            'from'     => $this->toAddressWithNameAndEmail($header->from[0]),
            'date'     => $header->date,
            'subject'  => $subject,
            'uid'       => $uid,
    ...
    private function toAddress($headerinfos) {
        ...
        */
        $addressInfo=$this->toAddressWithNameAndEmail($headerinfos);
        
        
        return $addressInfo['name'] . " <" . $addressInfo['email'] . ">";
    }
     ...
     * @param $headerinfos the infos given by imap
     */
    private function toAddressWithNameAndEmail($headerinfos) {
