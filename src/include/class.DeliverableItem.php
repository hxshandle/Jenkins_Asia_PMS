...
class DeliverableItem {
  ...
  function getItem($id) {
    $id = (int) $id;
    $sel = mysql_query("select * from deliverable_id where ID = $id");
    $item = mysql_fetch_array($sel);
    if (!empty($item)) {
      $taskl = mysql_query("select * from tasks where deliverable_item = $item[ID]");
      $taskList = mysql_fetch_array($taskl);
      if (!empty($taskList)) {
        $item['tasks'] = $taskList;
      } else {
        $item['tasks'] = array();
  ...
  function getDeliverableItemsByPhaseId($phaseId, $limit = 10) {
    $ret = array();
    $phaseId = (int) $phaseId;
    $sel = mysql_query("select * from deliverable_item where phase = $phaseId LIMIT $limit");
